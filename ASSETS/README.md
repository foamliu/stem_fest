...
ASSETS/SCENES/03_classroom_day/classroom_day_wide_v01.png
...
| 02 | 刘思成 | `02_liu_sicheng/liu_sicheng_hero_v01.png` | PNG 2304×1728 RGB（横版） | **18（唯一单人镜）** + 53 个多人镜（同上） | **女**（★ 2026-09-19 读图实测复核，见下方说明） | 同一位初中女生，黑色中长发扎马尾，细框眼镜，白色短袖 Polo 衫，黑色长裤，红领巾 | 待填 |
...
> ⚠️ **四小强的性别（2026-09-19 更正）**：**刘思齐与刘思成都是女生**（两人定妆照近乎同款：
> 黑色中长发扎马尾、细框眼镜、白色短袖 Polo + 红领巾）；**张书扬与徐畅景是男生**。
>
> 上表「性别」列**原用镜 16「学**她**」当判据，但现版镜 16 已无此台词** ⇒ 已改为「读图实测复核」。
> **判据必须来自定妆照本身，不能来自某句台词或复盘叙事** —— 当年正是从
> 「镜 61 的 prompt 写的是'初中女生'」**倒推**出「刘思成 = 男生」，写进了
> `_readme_history.md` §6.10.4 与 `_diag_act2_startup.py` 的注释，
> 于是镜 17/18（及镜 61/63）的 prompt 写成「初中**男生**（**不戴眼镜**）」，
> 与女生定妆照打架 ⇒ H3 两边都不听，配出**男声**、画出不戴眼镜的男生 ——
> 即用户反馈的"镜 17/18 女声变成男声"。这正是本文件开头「第一条铁律」要防的
> **"让剧本文字说话"**，而 `README.md` 铁律 B6 也正是这一类冲突的解法。
>
> ⇒ **修法**：四镜 prompt 改回「**戴细框眼镜的初中女生**」；**验收必须 `qwen3_asr` 回读确认女声**。
> 📌 本轮只改文档与 prompt 文本，尚未重跑生成。
...
├── SCENES/     # 7 个场景目录 + _school_raw/（17 张实景原片池，分类见 00_SOURCE.md）
│               #   ⚠️ 2026-09-21 删 `04_classroom_dusk`（并入 `03_classroom_day`）
├── PROPS/      # 12 个道具目录
...
| 03 | `03_classroom_day` | 八（1）班教室（**傍晚 6 点，天还亮**） | 5-9、**10-18** | 下午 → 傍晚 | **暖金色天光 + 蓝紫冷光**（镜 10-18 全息设备冷光） |
| ~~04~~ | ~~`04_classroom_dusk`~~ | ~~八（1）班教室（傍晚 6 点）~~ | ~~10-18~~ | — | **2026-09-21 合并进 `03` 并物理删除**（用户：故事开始就是现实世界的下午 6 点，本来就是 dusk，三套教室过于冗余） |
| 05 | `05_classroom_night` | 八（1）班教室（夜） | 106-126 | 现实只过了一小会儿 | 深蓝 + 橙红、屏幕光打脸 |
| 06 | `06_trench` | 上甘岭战前交通壕 | 19-46 | 1952-10~11，风雪阴冷 | **焦土 + 薄雪 + 泥水 + 冷灰蓝**（★ `trench_wide_v01~v04` 四张均为**风雪阴冷版**，2026-09-20 用户重生成） |
...
> ★ **2026-09-21 教室场景合并**：全片教室只剩**两种光** —— `03` = 傍晚 6 点天还亮（暖金，覆盖镜 5-18）
> / `05` = 夜（深蓝 + 橙红，覆盖镜 106-126）。原 `04_classroom_dusk`（镜 10-18）已删并入 `03`。
...
> ★ **2026-09-21 新增：场景多版本务必写「定版 + 理由」**
> 用户重做过场景图的四个目录（`03_classroom_day` v01~v05 / `05_classroom_night` v01~v04 /
> `07_rice_field` v01~v04 / `08_train_dining` v01~v05）**各留了一行 `README.txt`**，
> 写明 **定版 = v0X** 与**为什么**（照 `06_trench` 立的规矩）。
>
> ⚠️ 起因：脚本里 `SCENE` 是**写死文件名**的 —— 若只把新图放进目录、不改脚本，
> **成片不会变**（素材白换）。自查命令：
> ```
> py -3.10 OUTPUT/_check_scene_refs.py     # 校验 6 个幕脚本的场景图引用是否都指向真实文件
> ```
...
| 03 教室·傍晚 | ★ `03_classroom_day/classroom_day_wide_v01.png` | 2848×1600 | 豆包图生图（源：用户教室照）。**2026-09-20 用户重生成 v01~v05**：均为**同一间教室的不同视角**（差异在机位/景别，不在光线），全套「傍晚 6 点、天还亮、暖金光」基调一致。<br>★ **定版 = v01**（2026-09-21 用户指定「第一张」）：正对讲台的标准全景，后墙板报 + 左窗 + 右窗日落信息最全，适合镜 10-18 的多人调度；v04 = 走廊隔窗的外视角，v02/v03/v05 = 偏近/偏侧中景。<br>**本目录同时覆盖原 `04_classroom_dusk` 的镜 10-18**（该目录 2026-09-21 已删，产物见 `OUTPUT/03_classroom_day/`，迁移脚本 `OUTPUT/_migrate_04_to_03.py`）。 |
| 05 教室·夜晚 | ★ `05_classroom_night/classroom_night_wide_v01.png` | 2848×1600 | 豆包图生图。**2026-09-20 用户重生成 v01~v04**：均**同一间夜教室的不同视角**（夜色 + 空桌椅 + 后墙板报 + 暖橙壁灯一致）。<br>★ **定版 = v01**（2026-09-21 用户指定「第一张」）：四张里唯一**把「冷光笔记本屏幕」放在主体位置**的机位（左前方），正对尾声镜 106-126「熬夜攻关」的戏；v02 = 纯空镜全景，v03 = 偏右近景，v04 = 隔门框外视角。 |
| 06 战壕 | ★ `06_trench/trench_wide_v04.png` | 2848×1600 | **2026-09-20 用户重生成的风雪阴冷版**（`v01~v04` 四张全部替换，均可用；v02 雪最多、v04 最暗最冷）。<br>实测：亮度 79.8~84.9 / **R-B −18.1~−20.4（冷调）** / 高亮雪像素 3.5~5.2%。<br>场景内容：原木加固壕壁 + 沙袋垛 + 弹药木箱 + 丢弃的铁锹 + 泥水洼 + 薄雪。<br>★ 起因（**推翻旧稿**）：用户指出旧稿「深秋有太阳、无积雪」与史实不符 —— 已核实两条硬史料：① 「爆尘、浓烟遮天蔽日，老兵以为那天是阴天」（张嵩山，中国网络电视台）；② 「零下二三十摄氏度的冬天，战士们凿筑坑道」（新华网 2022-11-25）⇒ 战役 43 天跨 10-11 月，风雨雪与阴冷**都是史实**。<br>⚠️ 旧记录「v04 = 深秋调色锁定版（`_diag_grade_trench_autumn.py` 暖化，R-B 12.9→24.4）」**已作废** —— 同名文件被新图覆盖 |
| 07 稻田 | ★ `07_rice_field/rice_field_wide_v01.png` | 1920×1080 | **2026-09-20 用户重生成 v01~v04**：`z_image_turbo_t2i` seed **3002**，均**同一片稻田的不同视角**。<br>★ **定版 = v01**（2026-09-21 用户指定「第一张」）：大远景 + 田垄纵深引导线 + 左侧老式砖木校舍，最适合带人物的镜 47-74；v02/v03 = 稻穗前景特写（空镜用），v04 = 逆光剪影 + 大树单株（氛围镜）。<br>创作思路见该目录 `README.txt`（地貌=湘西雪峰山丘陵/沅江流域、建筑=砖木校舍土坯农房、试验田有边界感、胶片年代感、无人空镜）。 |
| 08 餐车 | ★ `08_train_dining/train_dining_wide_v01.png` | 2848×1600 | **2026-09-20 用户重生成 v01~v05**：`z_image_turbo_t2i` seed **3003**，均**同一节高铁餐车的不同视角**（红绒座椅 + 顶部灯带 + 白色箱堆 + 吧台台面一致）。<br>★ **定版 = v01**（2026-09-21 用户指定「第一张」）：车厢一端望向另一端的纵深全景，能同时容纳「多人群像 + 车窗夜色」，适合镜 75-105；v02~v05 = 座椅/吧台/箱堆的局部视角。 |
