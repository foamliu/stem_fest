...
★ 镜 1「校牌」的正解（2026-09-19 第三轮，★ 与前两轮方向相反）：
  参考图 `school_gate_wide_v02.png` 门柱上**本来就有**一行大号金色立体校名
  （红色圆徽在上、「上汇实验学校 / Shanghai Experimental School」金色立体字在下），
  字高约 38px ⇒ **R2V 抄得动**。前两轮为了躲「抄错字」把墙面抹成白板，
  结果**校名消失 + 构图退化成墙面纹理特写**，反而丢了展示功能。
  ✅ 现在改为：**正向要求照抄参考图原有的校徽与校名**，
     只禁止「参考图里**没有的**额外文字」。详见 TASKS[1] 上方注释与 README §6.9 的降级说明。
...
TASKS[1] = dict(
    ...
    prompt=(
        ...
        "<Picture 2> 里那面**竖条纹木饰面门柱**、柱上**红色圆形校徽**与它下方那行**金色校名立体字**，"
        "以及深色金属大门与门柱 —— 不要贴到墙面上拍特写。"
        "★★ 门柱上那枚红色圆形校徽、以及它正下方那一行**金色中英文校名立体字**，"
        "严格照 <Picture 2> 原样保留：位置、字号、颜色、排版都不变，"
        "字形要清晰端正、笔画完整，**中文字与英文字母都不得拼错或糊成一团**；"
        "★ 除参考图里已有的这组校名与校徽之外，画面上**不得再出现任何其它文字** —— "
