...
★ 本幕的三个硬约束（storyboard 明写「不许压 / 不许删」）：
  · 镜 4 的 **7s**（`HOLDS[4]` 已锁定）—— 需依次掠过操场 / 林荫道 / 花坛 / 教学楼四地点，
    每地 1.5-2s。航拍跟拍 ⇒ **steps=20**
  · 镜 6 的 **5s**，台词含「是不是隔壁班暗恋我的？」—— 建立张书扬性格。
    ⚠️ 本镜**没有 `HOLDS` 兜底**，按公式只有 2.33s ⇒ **压时长就等于压掉这句台词**
  · 镜 1/4/5 为**上汇实验学校展示镜**（校牌 / 校园四地点 / 窗外校园一角）—— 不许压

★ 镜 1「校牌」的正解（2026-09-19 **第四轮**；前三轮方向已全部推翻，见 README §6.9）：
