...
GUARD_HUANG = (
    ...
    "（**军绿色立领军装 + 军帽上的红色五角星帽徽必须保留**，"
    "不可改成其他军装、不可摘下军帽）"
)
GUARD_GLASSES = "（**细框眼镜必须保留**）"

# 无台词镜的通用禁语音后缀（2026-09-13 按 README §4.4 重写：
# 旧版写"不要出现任何语音、拟声词"与正文要求的"轻笑声/气音"自相矛盾，
# 而 H3 遇到矛盾时只会挑它擅长的做 —— 即产人声）。
NO_SPEECH = (
...
TASKS[19] = dict(
    ...
    prompt=(
        ...
        "Audio: 【本镜不要生成任何可辨认的语音或对白。只允许环境音与物件音；"
        "若画面有人群，也只是听不清的杂音，不要生成任何词句。】"
    ),
)
...
TASKS[26] = dict(
    ...
    prompt=(
        ...
        "Audio: 风声（后期）；女生平静而认真地说：我们是从未来来的。76年后。"
    ),
)
