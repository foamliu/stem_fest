...
台词与镜号严格取自 `storyboard.md` §第一幕：上甘岭（战前）（1952-10 朝鲜战场交通壕，深秋干冷）。
...
路线（与序幕一/二一致，README §7）：
  · 有角色的镜 → `video_minimax_h3_r2v`
      ...
      ref_image_2 = SCENES/06_trench/trench_wide_v04.png → <Picture 2>
        （★ 该图是 **2026-09-20 深秋调色版·终版**：_diag_grade_trench_autumn.py
          把 v02 的灰绿阴天数值暖化成「深秋枯黄、有太阳」（R-B 12.9→**24.4**、
          G/R 1.083→**1.0005**；地面亮度 54→64，构图与道具零改动）。
          **勿换回 v01/v02/v03** —— 灰绿或发闷的参考图会把 H3 带出夏景。）
...
#
# ★ 2026-09-19 史实修正（用户指出「上甘岭这一战是在 10 月份，场景不对」）：
#   史实依据（用户检索核对）：
#     · 上甘岭战役 = 1952-10-14 — 11-25。打响当天是深秋，早晨寒意未褪，
#       但白天有太阳（有记载当天「艳阳高照」）。
#     · 严寒（-20 ~ -39℃、冻土一米、大雪狂风）属于 11 月下旬战役后期
#       及 1953 年初后续部队接防时期，不是 10 月。
#     · 志愿军着装 = 「50式」冬服：棉衣（战士款双肩护肩布、袖口袢带）、
#       棉裤（裤脚抽带）、栽绒棉帽（带听孔/可放护耳）、高腰毛皮鞋。
#   ⇒ 旧版写「天色偏冷的、没有直射阳光」，把深秋阴天误写成隆冬阴霾，与史实不符。
#     改为：深秋、天色偏冷但白天有太阳、光线偏硬偏斜，地面无积雪、草木枯黄。
#   ⚠️ 四小强仍穿现代校服（白短袖 Polo + 红领巾）：他们是 2026 年进入世界模型的
#      穿越者，按设定不应换装 —— 这是刻意的时代对比，不是错误。
#      需要修正的是志愿军战士的着装（见 GUARD_SOLDIER_WINTER）与整体气候。
# ★ 2026-09-20 二次修正（用户复审：镜 21 仍是「绿油油的夏天」，与镜 19 的秋色自相矛盾）：
#   根因 = 旧 LIGHT 只写了「草木已经枯黄」（一句带过），而场景参考图 <Picture 2> 里
#   却有**返青的湿泥与鲜绿草丛**（该图 09-12 生成，调色是灰绿色的），H3 在
#   「文字 vs 参考图」冲突时倾向照抄参考图 ⇒ 出夏景。见 README §6.10.3 锚点冲突教训。
#   修法 = 把「枯黄」从一句提升为**可执行的多条视觉禁令**（正向描述，不用否定祈使——
#   否定词会被 H3 渲染成字幕，见 _verify_prompt_clean.py）：
#     · 明确地面状态：干硬的黄褐色土地、浮土与碎石（堵「湿泥水洼」）
#     · 明确植被状态：枯黄发干的野草、落叶、光秃的枝条（堵「鲜绿草丛/繁茂绿植」）
#     · 明确光线状态：低角度斜射的太阳、硬朗清晰的影子（堵「阴天漫射」）
#     · 明确时令对比：画面前景不要出现鲜绿茂盛的植物（该句为本幕**新增**的时令锚点）
#   ⚠️ 四小强仍穿现代校服（白短袖 Polo + 红领巾）：他们是 2026 年进入世界模型的
#      穿越者，按设定不应换装 —— 这是刻意的时代对比，不是错误。
#      需要修正的是志愿军战士的着装（见 GUARD_SOLDIER_WINTER）与整体气候。
LIGHT = (
    "1952 年 10 月朝鲜战场，深秋时节：天色偏冷、通透，白天有太阳，"
    "光线偏硬、从侧上方斜射过来，地面上没有积雪；"
    "草木已经枯黄，被炸松的焦土、沙袋与弹坑构成背景，空气干冷、能见度尚好。"
    "地面是干硬的黄褐色土地，浮着一层干燥的浮土与碎石，没有积水与水洼；"
    "植被是枯黄发干的野草与落叶、枝条光秃，画面前景没有鲜绿茂盛的植物"
)

# ★ 2026-09-20 三次修正：场景图由 v02（灰绿阴天）换成 v04（深秋枯黄、有太阳）。
#   理由：prompt 已写明「枯黄/有太阳」，但 H3 会**照抄参考图**的色温（README §6.10.3
#   锚点冲突）—— 镜 21 出夏景就是文字与图冲突的直接后果。图与文字必须同时变秋。
#   v04 = `_diag_grade_trench_autumn.py` 对 v02 做两档数值暖化
#        （R-B 12.9→**24.4**、G/R 1.083→**1.0005**；地面亮度 54→64），
#   构图/道具/壕形零改动 ⇒ 28 个镜的空间与角色锁定不受影响。
#   ⚠️ v03 是中间档（R-B 26.8 / G/R 1.007）但地面亮度只有 66「发闷」，
#      与 LIGHT 的「白天有太阳、光线偏硬」仍不匹配 ⇒ 弃用，直接用 v04。
...
GUARD_SOLDIER_WINTER = (
    ...
    "帽徽是红色五角星；脚穿高腰毛皮鞋；画面里没有穿单衣、戴单帽、露出手臂或脖子的人。"
)
...
TASKS[19] = dict(
    ...
    prompt=(
        ...
        "环境是 <Picture 2> 那条深秋的战前交通壕：土壁、沙袋、弹坑、稀稀拉拉的野草；"
...
TASKS[20] = dict(
    ...
    prompt=(
        ...
        "背景是 <Picture 2> 那条深秋的战前交通壕，土壁与沙袋，壕壁上枯草发黄、枝条光秃；"
...
TASKS[21] = dict(
    ...
    prompt=(
        ...
        "表情是好奇加嫌弃；那丛野菜叶片发干、边缘卷起、颜色暗绿发黄，是深秋霜冻前最后一点野菜；"
        "右边那位（男生，不戴眼镜）站在他身后侧；"
        "两人的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那条深秋的战前交通壕，土壁上只长着稀稀拉拉、枯黄发干的野草；"
...
TASKS[22] = dict(
    ...
    prompt=(
        ...
        "右边那位（男生）还保持着蹲着的姿势；"
        "两人的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那条深秋的战前交通壕，土壁与枯黄发干的野草；"
        + LIGHT + "。镜头固定不动。\n"
...
TASKS[23] = dict(
    ...
    prompt=(
        ...
        "三人的面貌、发型与服装严格照 <Picture 1>" + GUARD_HUANG + "；" +
            GUARD_SOLDIER_WINTER +
        "背景是 <Picture 2> 那条深秋的战前交通壕，土壁与沙袋；"
...
TASKS[31] = dict(
    ...
    prompt=(
        ...
        #        ② 正向写「光线来源」而不是只写「深秋干冷」；
        #        ③ 换 seed（旧 seed 落在坏支上）。
        "CUT 1: 深秋的战前交通壕里，土壁与沙袋堆成的壕壁占满背景，"
...
TASKS[37] = dict(
    ...
    prompt=(
        ...
        "背景是 <Picture 2> 那条深秋的战前交通壕，土壁虚化，壕壁上枯草发黄；"
        + LIGHT + "。镜头缓慢向前推近（slow dolly in）。\n"
...
TASKS[38] = dict(
    ...
    prompt=(
        ...
        + GUARD_HUANG + "；" +
            GUARD_SOLDIER_WINTER +
        "背景是 <Picture 2> 那条深秋的战前交通壕，土壁完全虚化；"
...
TASKS[39] = dict(
    ...
    prompt=(
        ...
        "镜头落在这两只握着的手上，两人的手与军装严格照 <Picture 1>" + GUARD_HUANG + "；" +
            GUARD_SOLDIER_WINTER +
        "背景是 <Picture 2> 那条深秋的战前交通壕，土壁与沙袋完全虚化；"
...
TASKS[41] = dict(
    ...
    prompt=(
        ...
        "环境是 <Picture 2> 那条深秋的战前交通壕，土壁、沙袋与弹坑，壕壁上只有枯黄发干的野草；"
        + LIGHT + "。镜头固定不动，留出一秒安静。\n"
...
TASKS[42] = dict(
    ...
    prompt=(
        ...
        "两人的面貌、发型与服装严格照 <Picture 1>" + GUARD_HUANG + "；" +
            GUARD_SOLDIER_WINTER +
        "背景是 <Picture 2> 那条深秋的战前交通壕，土壁与硝烟；"
...
TASKS[45] = dict(
    ...
    prompt=(
        ...
        "五人的面貌、发型、眼镜与服装严格照 <Picture 1>" + GUARD_HUANG + "；" +
            GUARD_SOLDIER_WINTER +
        "环境是 <Picture 2> 那条深秋的战前交通壕，土壁、沙袋与延伸向远处的壕沟；"
...
TASKS[46] = dict(
    ...
    prompt=(
        "CUT 1: 纯白画面转场。画面从一片深秋干冷的战壕色调迅速被刺眼的白光完全吞没，"
