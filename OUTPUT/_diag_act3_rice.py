...
⚠️ 命名说明（2026-09-13 澄清）：文件里的 "act3" / `07_rice_field` 是**按 scene 目录序号**命名，
   与「第几幕」无关。全片映射如下，别再混：
       ...
       序幕二《启动》    镜 10-21  → 03_classroom_day (_diag_act2_startup.py)
...
#   v01~v04 都是**安江农校稻田的同一场景、不同视角**（差异在机位与景别，不在年代与地貌）。
#   定版依据「本幕需要什么」：镜 47-74 的关键调度是**人物在田垄间行走/俯身/远眺校舍**，
#   v01 = **大远景 + 田垄纵深引导线 + 左侧老式砖木校舍全貌**，一眼交代「农校的试验田」这层含义，
#         也是带人物入画时唯一不会把人挤到边缘的构图；
#   v02/v03 = 稻穗贴脸的前景特写（空镜可用，带人就会挡住脸）；v04 = 逆光剪影 + 大树单株（氛围镜）。
...
# ★ 2026-09-16 补（README §6.10）：镜 57 的 storyboard 参考图提示是「张书扬+袁隆平二人合影」，
#   旧版只喂 P_ZHANG（张书扬单人照）⇒ 袁隆平根本不在画面里。
#   组合早就预制在 `_make_group.py` 的 COMBOS 里，只是本脚本没接线。
G_ZHANG_YUAN = os.path.join(FRAMES, "_group", "zhang_shuyang_yuan_longping_hero_v01.png")
...
TASKS[47] = dict(
    ...
    ref1=G_FOUR, ref2=SCENE_RICE, dur=4.0,
    prompt=(
        # ★ 2026-09-16 修复「景别漂移」（README §6.10）：
        #   storyboard 定**远景**（参考图提示明写「远景，人物小」），
        #   旧版 prompt 写「中景四人镜头」⇒ 成片人物占满画幅、稻田环境被挤没，
        #   与「第二幕幕首建立镜（交代稻田环境）」的功能不符。
        #   修法：改「远景」并把**环境描写提到人物之前**，人物明确写小、环境占主导。
        "CUT 1: 远景镜头，画面以 1961 年 7 月盛夏的稻田为主："
        "金黄与翠绿交叠的稻浪从近处一直铺到天际线，田埂蜿蜒伸向画面深处，"
        "远处是安江农校的低矮建筑与山影；"
        "画面中景偏下有四个人物 —— 四位穿白色短袖 Polo 衫系红领巾的初中生，"
        "在这幅远景里很小、只占画面高度的六分之一左右，并肩站在田埂上、背对镜头略微侧身，"
        "一起望向画面远处；更远处的稻田里另有一个更小的人影，"
        "是一位穿深灰中山装的中年男人正弯腰在稻田里翻找稻子（几乎只见背影与草帽）；"
        "四人的面貌、发型、服装严格照 <Picture 1>；"
        "环境是 <Picture 2> 那片 1961 年 7 月盛夏的稻田：稻浪被风吹动、田埂泥土、远处农校建筑；"
        + LIGHT + "。镜头缓慢横摇扫过整片稻田（slow pan），稻叶随风摆动。\n"
        "Audio: 蝉鸣、风吹稻浪的沙沙声（后期）；男生抱怨：热死了。"
    ),
)

TASKS[48] = dict(
    slug="xu_changjing_looks_far", seed=9553,
    ref1=P_XU, ref2=SCENE_RICE, dur=3.0,
    prompt=(
        ...
        "Audio: 蝉鸣与风吹稻浪声（后期）；男生说：安江农校。一九六一年。"
    ),
)
...
TASKS[49] = dict(
    ...
    ref1=P_LIU_SIQI, ref2=SCENE_RICE, dur=3.0,
    prompt=(
        "CUT 1: 近景半身镜头，人物占画面高度约三分之二。"
        ...
        "Audio: 蝉鸣、稻浪声（后期）；女生问：那个人在干什么？"
    ),
)

TASKS[50] = dict(
    slug="yuan_bending_looking_for_rice", seed=9555,
    ref1=P_YUAN, ref2=SCENE_RICE, dur=3.0,
    prompt=(
...
TASKS[51] = dict(
    ...
    ref1=P_LIU_SICHENG, ref2=SCENE_RICE, dur=4.0,
    prompt=(
        "CUT 1: 近景半身镜头，人物占画面高度约三分之二。"
        ...
        + LIGHT + "。镜头轻微横移（slow lateral move）。\n"
        "Audio: 蝉鸣、稻浪声（后期）；女生说：他在找东西。一株一株地找。"
    ),
)

TASKS[52] = dict(
    slug="rice_plant_eyes_light_up", seed=9557,
    ref1=None, ref2=None, dur=3.0,
    prompt=(
...
TASKS[53] = dict(
    ...
    ref1=P_YUAN, ref2=SCENE_RICE, dur=2.0,
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：storyboard 定**特写**、运镜「固定」，
        #   旧版写「近景 / 缓推」⇒ 成片看不到「抚摸稻穗」的细节（本镜的信息点就是这株稻子）。
        "CUT 1: 特写镜头，画面主体是一只手与一株稻穗："
        "一位穿深灰中山装的中年男人用手掌轻轻抚摸一株稻穗，"
        "手指缓缓从穗尖滑过，饱满的稻粒清晰可辨；"
        "他的手与稻穗占满画面，只见他的下颌与衣领（中山装）；"
        "他的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田的绿色虚化；"
        + LIGHT + "。镜头固定不动，只留轻微手持呼吸感。\n"
        "Audio: 蝉鸣与稻浪声（后期）；他说：找到了。"
    ),
)

TASKS[54] = dict(
    slug="yuan_stands_waves_them_over", seed=9559,
    ref1=G_FOUR_YUAN, ref2=SCENE_RICE, dur=4.0,
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：storyboard 定**全景**，旧版写「中景镜头」
        "CUT 1: 全景镜头，五人全身入画、稻田环境占画面大部分："
        "一位穿深灰中山装的中年男人从稻田里直起腰，"
        "转头看见站在田埂上的四位初中生，抬手向他们招呼、示意走近，脸上带着朴实的笑；"
        "五人的面貌、发型与服装严格照 <Picture 1>；"
        "环境是 <Picture 2> 那片盛夏稻田，他站在稻丛中、四位学生站在田埂上；"
        + LIGHT + "。镜头轻微横移（slow lateral move）把他与田埂上的四人一并纳入。\n"
        "Audio: 蝉鸣、稻浪声（后期）；他说：过来嘛，站那么远做啥子？"
    ),
)
...
TASKS[55] = dict(
    ...
    prompt=(
        ...
        #   ① storyboard 定**全景**、运镜「跟移」，旧版写「中近景 / 缓推」；
        #   ② storyboard 参考图提示是「**袁隆平+四人合影**」，旧版只喂了袁隆平单人照
        #      ⇒ 四人被挤出画面。修法：改全景 + ref1 换四人合影。
        "CUT 1: 全景镜头，五人与整片稻田同时入画："
        "一位穿深灰中山装的中年男人从田里拔起一株普通稻子，"
        "举在身前给身旁围着的四个人看，另一只手指着稻穗，神情认真地讲解；"
        "四位穿白色短袖 Polo 衫系红领巾的初中生站在田埂上、身子微微前倾地听着；"
        "五人的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田；"
        + LIGHT + "。镜头跟移（follow），跟着他举起稻子的手轻微移动。\n"
        "Audio: 蝉鸣、稻浪声（后期）；他说：你们看，这是田里最普通的禾苗。"
    ),
)

TASKS[56] = dict(
    slug="yuan_points_marked_ear", seed=9561,
    ref1=P_YUAN, ref2=SCENE_RICE, dur=4.0,
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：storyboard 定**中景**、
        #   运镜「固定，轻微下摇」；旧版写「中近景 / 缓推」。
        "CUT 1: 中景镜头。一位穿深灰中山装的中年男人伸手指向稻田里一株系着浅色布条的稻穗，"
        "神情郑重；画面中那株稻子比其他稻子明显高大、穗子更饱满，布条随风轻动；"
        "他的上半身与那株稻穗同框；"
        "他的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田；"
        + LIGHT + "。镜头固定不动，轻微下摇（slight tilt down）落到那株稻穗上。\n"
        "Audio: 蝉鸣、稻浪声（后期）；他说：那株，我找了三年才找到。"
    ),
)

TASKS[57] = dict(
    slug="zhang_shuyang_leans_in", seed=9562,
    ref1=G_ZHANG_YUAN, ref2=SCENE_RICE, dur=4.0,
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：storyboard 定**中近景**、运镜「固定」；
        #   旧版写「近景 / 缓推」，且 ref1 只喂张书扬单人照 ⇒ 袁隆平不在画面（storyboard 要求二人同框）。
        "CUT 1: 中近景双人镜头，一位初中男生与一位穿深灰中山装的中年男人同框、"
        "处在同一光照环境、画面是完整连续的空间："
        "男生凑近仔细看那株系着布条的稻穗，眉毛挑起、表情惊讶好奇，"
        "一只手撑在膝盖上、身体前倾；中年男人站在他旁边、手指仍指着那株稻子；"
        "两人的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田的绿色虚化；"
        + LIGHT + "。镜头固定不动，只留轻微手持呼吸感。\n"
        "Audio: 蝉鸣、稻浪声（后期）；男生惊讶问：您找了三年？就为了田里的一株禾苗？"
    ),
)
...
TASKS[58] = dict(
    ...
    ref1=P_YUAN, ref2=SCENE_RICE, dur=3.0,
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：storyboard 定**中景**；旧版写「近景」。
        "CUT 1: 中景镜头。一位穿深灰中山装的中年男人笑着看向画面外的四个孩子，"
        "笑容憨厚、眼角有细纹，手里还捏着一株稻穗；他的上半身与周围稻丛同框；"
        "他的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田的绿色虚化；"
        + LIGHT + "。镜头轻微横移（slow lateral move）。\n"
        "Audio: 蝉鸣、稻浪声（后期）；他问：你们吃饭了没？"
    ),
)

TASKS[59] = dict(
    slug="liu_siqi_answers_quietly", seed=9564,
    ref1=P_LIU_SIQI, ref2=SCENE_RICE, dur=2.0,
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：
        #   ① 时长 5s→3s（storyboard 定 3s，旧值违反时长列）
        #   ② 加「初中生年龄/校服」硬锚点 —— 旧版成片出现**粉色领校服**，
        #      与全片白短袖 Polo + 红领巾不统一（`<Picture 1>` 是单人定妆照，
        #      H3 会按照片里的实际服装出图，必须用文字钉死校服款式）
        "CUT 1: 近景半身镜头，人物占画面高度约三分之二。"
        "一位戴细框眼镜的初中女生（十二三岁的初中生年龄、"
        "穿白色短袖 Polo 衫、胸前系红领巾）低着头、小声回答，神情有点拘谨，"
        "手指微微攥着衣角；"
        "她的面貌、发型、眼镜与服装严格照 <Picture 1>（细框眼镜必须保留；"
        "校服必须是白色短袖 Polo 衫 + 红领巾）；"
        "背景是 <Picture 2> 那片盛夏稻田的绿色虚化；"
        + LIGHT + "。镜头轻微向前缓推（slow dolly in）。\n"
        "Audio: 蝉鸣、稻浪声（后期）；女生说：吃了。"
    ),
)

TASKS[60] = dict(
    slug="yuan_talks_about_hungry_people", seed=9565,
    ref1=P_YUAN, ref2=SCENE_RICE, dur=8.0,
    prompt=(
        "CUT 1: 特写镜头，画面只取一位穿深灰中山装的中年男人的面部："
        "他点点头，目光望向稻田远方，神情平静而认真，说话时；"
        "他的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田，稻浪在风里起伏、轻微虚化；"
        + LIGHT + "。镜头轻微向前缓推（slow dolly in）。\n"
        "Audio: 蝉鸣、稻浪声（后期）；他说：那就好。我以前见过饿倒的人。所以想让禾苗多结一点粒实。"
    ),
)

TASKS[61] = dict(
    slug="liu_sicheng_blurts_out", seed=9589,
    ref1=P_LIU_SICHENG, ref2=SCENE_RICE, dur=3.0,
    prompt=(
        # ★ 2026-09-19 更正（README 铁律 B6）：**刘思成是女生**。
        #   2026-09-16 那次「修复」从 prompt 文字倒推外形，把性别判反了
        #   ⇒ 写成「初中男生（不戴眼镜）」，与女生定妆照冲突、成片配出男声。
        #   ⇒ 文字改回「戴细框眼镜的初中女生」；年龄 + 校服锚点保留（那才是当年真该改的）。
        "CUT 1: 近景半身镜头，人物占画面高度约三分之二。"
        "一位戴细框眼镜的初中女生（十二三岁的初中生年龄、"
        "穿白色短袖 Polo 衫、胸前系红领巾）脱口而出、刚开口就顿住，"
        "神情有点慌张又有点激动；"
        "她的面貌、发型与服装严格照 <Picture 1>（校服必须是白色短袖 Polo 衫 + 红领巾）；"
        "背景是 <Picture 2> 那片盛夏稻田的绿色虚化；"
        "★ 全画面不得出现任何可读的文字、字幕、人名条或标识；"
        "画面上没有任何文字、字母或符号，只有人物与稻田的实拍画面；"
        + LIGHT + "。镜头轻微横移（slow lateral move）。\n"
        # ★ 2026-09-17 字幕泄漏修复（实测扫出「袁隆平爷爷」烧进画面）：
        #   §6.7 已证明「写守护句」挡不住，本例进一步证明**触发源是"喊出的专有名词"本身** ——
        #   H3 把「袁隆平爷爷」当成**说话人名牌**渲染了。
        #   本次仍加守护句（消除概率），但**主要靠重跑 + 读图复核**。
        "Audio: 蝉鸣、稻浪声（后期）；女生脱口而出：袁隆平爷爷，您……"
    ),
)
...
TASKS[63] = dict(
    ...
    ref1=G_FOUR_YUAN, ref2=SCENE_RICE, dur=4.0,
    prompt=(
        # ★ 2026-09-19 更正（README 铁律 B6）：**刘思成是女生**（原写「男生」= 判反）。
        "CUT 1: 中近景双人镜头，一位戴细框眼镜的初中女生与一位穿深灰中山装的中年男人同框、"
        "面对面站在稻田里，女生仰头认真地说着什么、神情坦诚，男人微微低头听着、表情一怔；"
        "两人的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田，绿稻与天光；"
        + LIGHT + "。镜头固定不动，两人处于同一光照环境、画面是完整连续的空间。\n"
        "Audio: 蝉鸣、稻浪声（后期）；女生说：袁隆平爷爷。我们是从未来来的。"
    ),
)
...
TASKS[65] = dict(
    ...
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：storyboard 定**中景**；旧版写「近景」。
        "CUT 1: 中景镜头，一位穿深灰中山装的中年男人的上半身与身后稻丛同框："
        "他迟疑地开口，眼神里有期待也有一丝不确定，微微前倾；"
        "他的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田的绿色虚化；"
        + LIGHT + "。镜头固定不动，只留轻微手持呼吸感。\n"
        "Audio: 蝉鸣、稻浪声（后期）；他问：那……现在大家能吃饱了？"
    ),
)
...
TASKS[66] = dict(
    ...
    ref1=P_LIU_SIQI, ref2=SCENE_RICE, dur=3.0,
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：时长 5s→3s（storyboard 定 3s）+
        #   补「初中生年龄 + 白短袖 Polo + 红领巾」校服锚点（旧版成片校服款式漂移）
        "CUT 1: 近景半身镜头，人物占画面高度约三分之二。"
        "一位戴细框眼镜的初中女生（十二三岁的初中生年龄、"
        "穿白色短袖 Polo 衫、胸前系红领巾）用力点头，眼眶泛红、一滴眼泪顺着脸颊掉下来，"
        "嘴唇动了动才把话说出口；"
        "她的面貌、发型、眼镜与服装严格照 <Picture 1>（细框眼镜必须保留；"
        "校服必须是白色短袖 Polo 衫 + 红领巾）；"
        "背景是 <Picture 2> 那片盛夏稻田的绿色虚化；"
        + LIGHT + "。镜头轻微向前缓推（slow dolly in）。\n"
        "Audio: 蝉鸣、稻浪声（后期）；她带着哭腔说：能吃饱了。"
    ),
)
...
TASKS[67] = dict(
    ...
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：storyboard 定**中景**；旧版写「近景」。
        "CUT 1: 中景镜头，一位穿深灰中山装的中年男人的上半身与身后稻丛同框："
        "他慢慢地笑了，笑得很轻、很平静，随后把目光重新投向脚下的稻田；"
        "他的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田的绿色虚化；"
        + LIGHT + "。镜头固定不动，只留轻微手持呼吸感。\n"
        "Audio: 蝉鸣、稻浪声（后期）；他说：那就好。那我接着找。"
    ),
)
...
TASKS[70] = dict(
    ...
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：storyboard 定**中景**；
        #   旧版写「近景」，且 ref1 只喂张书扬单人照 ⇒ 袁隆平不在画面。
        "CUT 1: 中景双人镜头，一位初中男生与一位穿深灰中山装的中年男人同框、"
        "处在同一光照环境、画面是完整连续的空间："
        "男生眼睛发亮、语气兴奋地向中年男人描述，抬手在半空比划出一个很高的高度，"
        "脸上是向往的神情；中年男人侧头看着他、听着；"
        "两人的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田的绿色虚化；"
        + LIGHT + "。镜头固定不动，只留轻微手持呼吸感。\n"
        "Audio: 蝉鸣、稻浪声（后期）；男生兴奋说：禾苗会像高粱那么高。您能在禾下乘凉。"
    ),
)
...
TASKS[72] = dict(
    ...
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：
        #   旧版只写「放进一位戴细框眼镜的初中女生的手心里」，
        #   但 ref1（刘思齐+袁隆平二人合影）里**有两双手**，
        #   特写下 H3 分不清哪只是谁 ⇒ 成片画出一双**成年男性的手**
        #   （与「初中生」设定冲突，且与镜 61/64 的成年男性连成一个错误人物）。
        #   修法：① 明写**两只手各属谁**（左手＝中山装长辈 / 右手＝戴眼镜的初中女生）
        #        ② 用手部特征区分年龄（粗糙的大手 vs 纤细的小手 + 蓝白校服袖口）
        "CUT 1: 特写镜头，画面中央是两只手：一位穿深灰中山装的中年男性的大手"
        "（指节粗大、皮肤被日头晒得偏黑）从稻穗上轻轻摘下一粒金黄色的稻谷，"
        "把它放进旁边一只初中女生的手里 —— 那只手小而纤细、手腕上露出"
        "白色短袖 Polo 衫的袖口与红领巾的一角（十二三岁初中生的手，"
        "绝不是成年人的手）；两只手在画面中央、动作很轻；"
        "穿中山装的长辈的面貌与服装严格照 <Picture 1>（中山装必须保留、不要改变长相）；"
        "初中女生戴细框眼镜、穿白色短袖 Polo 衫系红领巾；"
        "稻谷的形态与颜色照 <Picture 2>；"
