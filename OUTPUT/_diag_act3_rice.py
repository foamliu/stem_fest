...
TASKS[61] = dict(
    ...
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：
        #   ① 时长 5s→4s（storyboard 定 4s）
        #   ② **角色性别纠正**：刘思成是**男生**，旧版 prompt 写「女生」，
        #      与 <Picture 1>（刘思成定妆照）冲突 ⇒ H3 两边都不听，
        #      成片画出一个**穿深灰外套的成年男性**（与初中生设定冲突）。
        #      修法：文字改「男生」，并补「初中生年龄 + 白短袖 Polo + 红领巾」。
        "CUT 1: 近景半身镜头，人物占画面高度约三分之二。"
        "一位初中男生（十二三岁的初中生年龄、不戴眼镜、"
        ...
        "Audio: 蝉鸣、稻浪声（后期）；男生脱口而出：袁隆平爷爷，您……"
    ),
)
...
TASKS[63] = dict(
    ...
    ref1=G_FOUR_YUAN, ref2=SCENE_RICE, dur=4.0,
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：storyboard 定**中近景**、「固定双人」；
        #   旧版写「中景 / 横移」，且把刘思成写成「女生」（刘思成是男生）。
        "CUT 1: 中近景双人镜头，一位初中男生与一位穿深灰中山装的中年男人同框、"
        "面对面站在稻田里，男生仰头认真地说着什么、神情坦诚，男人微微低头听着、表情一怔；"
        "两人的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那片盛夏稻田，绿稻与天光；"
        + LIGHT + "。镜头固定不动，两人处于同一光照环境、画面是完整连续的空间。\n"
        "Audio: 蝉鸣、稻浪声（后期）；男生说：袁隆平爷爷。我们是从未来来的。"
    ),
)
