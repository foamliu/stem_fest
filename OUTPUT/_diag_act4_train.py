...
#   v01~v05 都是**同一节高铁餐车车厢的不同视角**，车厢陈设（红绒座椅、顶部灯带、
#   行李架白色箱堆、吧台不锈钢台面）与「夜 + 暖黄顶灯 + 窗外深蓝」基调完全一致。
#   定版依据：镜 75-105 的主体调度是**餐车吧台边的群像对话**（含钟南山的中近景），
#   v01 = **从车厢一端望向另一端的纵深全景** —— 红绒椅排 + 吧台 + 白色箱堆全在画面里，
#         是唯一能同时容纳「多人群像 + 车窗夜色」两种信息的机位；
#   v02~v05 = 更偏局部（座椅特写 / 吧台侧视 / 箱堆前景），单人群像镜可作补充视角。
...
# ★★ 2026-09-18 新增：**全画面文字禁令**（README §6.10.11 / §6.10.12）
#   根因（验收读图实锤）：镜 104 是"全景车厢"，画面里 H3 自发渲染了
#   **高铁车厢顶部的 LED 信息屏**，屏上是**不可读的伪汉字**
#   （红色「驩 忻苜鑧」+ 绿色「用」+ 彩色小图标），旁边端墙上还有一块
#   **带字的金属告示牌**。这类"场景 UI 文字"是 H3 对"高铁"这个概念的
#   训练语料记忆 —— 它不知道要留白，就自动补上了车厢应有的公告信息。
#   ⇒ 只写"不得出现文字"不够，必须**点名场景里会出现的文字载体**：
#     车厢顶部的 LED 信息屏 / 报站屏 / 车厢号牌 / 告示牌 / 广告位 / 时刻表 /
#     门牌 / 座位号 / 安全须知 / 灭火器标识 —— 全部写"不发光、无内容"。
NO_TEXT_SCENE = (
    "★ 全画面不得出现任何可读的文字、字幕、符号、数字或标识："
    "车厢顶部与车门上方没有任何发光的 LED 信息屏、报站屏或车厢号牌"
    "（那些位置就是空白的墙板，不发光、不显示任何内容）；"
    "端墙、隔板、门上、窗边、座位旁没有任何告示牌、广告、时刻表、"
    "安全须知、座位号、二维码或印刷物；"
    "桌面上的纸张是纯白的、空白无字的；"
    "画面内任何表面都没有 logo、商标、签名或水印；"
    "整个车厢干净得像未经印刷的实物照片。\n"
)
...
TASKS[97] = dict(
    ...
    prompt=(
        ...
        "Audio: 车轮与铁轨的节奏声（后期）；"
        "他说：一九五九年，我打破过全国四百米栏的纪录。中央体育学院想让我去国家队。"
    ),
)
...
# ── 镜 99 钟南山（★ 10s，全片最长台词；★ 必须用单人照，画面里出现别人会夺走说话权）──
#   ★ 事故复盘（2026-09-16）：旧版 ref1=张书扬+钟南山合影 → 画面里有张书扬 →
#     H3 把台词分配给了张书扬（说话人错）。修法：ref1 换成 solo_zhong_nanshan，
#     并在 prompt 明写"只有他一个人入镜、只有他一个人说话"。
TASKS[99] = dict(
    slug="zhong_fathers_words", seed=9725,
    ref1=G_ZN_SOLO, ref2=SCENE, dur=10.0,
    prompt=(
        "CUT 1: 近景镜头，画面里只有那位穿深色外套的老年男性一个人，"
        "他坐在高铁餐车的座位上、正面朝向镜头，"
        "他的嘴唇在动、正在念出这句台词，"
        "说的时候目光平稳、神情克制、没有炫耀的意思；"
        "整幅画面里只有他一位人物、没有其他任何人入镜；"
        "他的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那节高铁餐车车厢，暖黄顶灯 + 窗外夜色；"
        + LIGHT + "。镜头缓慢向前推。\n"
        "Audio: 车厢底噪与轻微的铁轨声（后期）；"
        "老年男性说：我父亲说，体育竞技不能成为从事一生的工作，"
        "但学医可以一辈子治病救人。"
    ),
)
...
#   ★ 事故复盘（2026-09-16）：旧版画面里车还在行进，人却已走出车门 —— 物理上矛盾。
#     根因：prompt 只写「高铁停稳，车门打开」，但 H3 对列车场景的**默认态就是"行进中"**，
#     于是把「行进」与「下车」叠加。修法 = ① 开篇第一句就写死"列车已经完全停稳、不再移动"，
#     ② 显式否定"窗外景物仍在移动"，③ 把「先停稳 → 再起身 → 再走向车门」写成明确时序。
TASKS[104] = dict(
    slug="zhong_walks_away_into_night", seed=9736,
    ref1=G_FOUR, ref2=SCENE, dur=5.0,
    prompt=(
        # ★★ 2026-09-18 修复（README §6.10.12 **车厢 UI 文字幻觉**）：
        #   验收读图实锤成片里出现**高铁顶部 LED 信息屏 + 伪汉字 + 带字告示牌**。
        #   本镜旧 prompt 是"全景车厢 + 无文字禁令"，H3 便把"高铁车厢"这个概念里
        #   记忆的公告信息全补上了。修法：
        #     ① 补 NO_TEXT_SCENE（点名 LED 屏/报站屏/告示牌/时刻表等载体）；
        #     ② 明写"车厢顶部与车门上方是空白墙板、不发光"；
        #     ③ 换 seed 9730 → 9736。
        "CUT 1: 全景镜头。高铁列车已经完全停稳在站台上、车体一动不动，"
        "窗外的景物是静止的站台灯光、不再有任何移动；"
        "车门已经打开。那位穿深色外套的老年男性先从座位上站起身，"
        "提起随身的小包，然后才穿过车厢走向打开的车门，"
        "背影一步步走远、最后消失在站台夜色的灯光里；"
        "整个过程中列车始终是停稳静止的，窗外没有任何掠过的景物；"
        "四位穿校服的初中生留在座位上、望着他的背影，没有起身；"
        + GUARD_ZN + "；"
        "四位学生的面貌、发型与服装严格照 <Picture 1>；"
...
# ── 镜 105 四人站在原地，画面泛白（全景；★ 白屏日记字转场）──
#   ★ 事故复盘（2026-09-16）：旧版是纯 T2V（ref=None），四小强**没有任何参考图** →
#     四个人脸全部自由发挥、与全片对不上（人物漂移）。
#     修法：改 R2V，ref1=四小强合影（four_students_hero_v02）；白屏日记字由后期烧字。
TASKS[105] = dict(
    slug="white_out_diary_card_act4", seed=9748,
    ref1=G_FOUR, ref2=SCENE, dur=5.0,
    prompt=(
        "CUT 1: 全景镜头。四位穿校服的初中生（两男两女）站在高铁餐车车厢里，"
        "望着车门外那个人消失的方向，一动不动、神情安静；"
        "画面从两侧开始逐渐泛白、白光一点点吞没整个画面，"
        "最后画面完全变成白屏（像一个过场转场的白光收束）；"
        "四位学生的面貌、发型与服装严格照 <Picture 1>"
        "（两男两女共四个人，一个都不能少、脸不能变）；"
        "四人全程保持静止：目光方向不变、面部肌肉放松、双唇自然闭合，就像四尊安静的雕像；"
        "背景是 <Picture 2> 的高铁餐车车厢暖黄灯光；"
        + LIGHT + "。固定机位。\n"
        # ★★ 2026-09-18：把泛泛的"不要出现文字"升级为**点名文字载体**的
        #   全画面禁令（同镜 104 根因：H3 会自发补上"高铁车厢应有的"LED 屏与告示牌）。
        #   旧写法「画面里不要出现任何文字、字幕或标识」是抽象禁令，
