...
#   v01~v04 都是**同一间夜晚教室的不同视角**（夜色 + 空桌椅 + 后墙板报 + 暖橙壁灯的基调一致）。
#   定版依据：镜 106-126 是**尾声**，关键信息有两条 —— ① 孩子们围着**发光笔记本**（屏幕冷光打脸）
#   ② 窗外已暗、室内只余暖橙余光。v01 = **正面偏左的全景，画面左前方就是那台亮屏笔记本**，
#   是四张里唯一**把「冷光屏幕」放在主体位置**的机位，正对尾声「熬夜攻关」的戏；
#   v02 = 更规整的正面全景（无人 + 无屏幕，纯空镜）；v03 = 偏右近景；v04 = 隔门框的外视角。
...
TASKS[106] = dict(
    ...
    prompt=(
        ...
        "一位戴细框眼镜的女生站在课桌旁，双手掌心向上托着两样半透明的、"
...
TASKS[112] = dict(
    ...
    ref1=G_FOUR, ref2=SCENE, dur=4.0,
    prompt=(
        "CUT 1: 近景镜头。一位初中男生看着屏幕，地念出一句话，"
        "表情是压着的难过与郑重；"
        "身后和身旁还站着另外三位同学，他们只入画一部分；"
        "四人的面貌、发型与服装严格照 <Picture 1>；"
        "背景是 <Picture 2> 那间傍晚已暗的教室，冷白的屏幕光；"
        + LIGHT + "。镜头缓慢向前推；★ 全画面不得出现任何可读的文字、字幕或符号。画面上没有任何文字、字幕或标识，人物脸上只有表演。\n"
        "Audio: 教室安静底噪（后期）；"
        "男生说：他问我们……咱们国家现在啥样了。"
    ),
)
...
TASKS[117] = dict(
    ...
    ref1=G_FOUR, ref2=SCENE, dur=4.0,
    prompt=(
        "CUT 1: 中景镜头。一位初中男生从座位上站起来，抬头看着面前的全息屏幕，"
        "站姿笔直、双手自然垂下；"
...
TASKS[118] = dict(
    ...
    ref1=G_FOUR, ref2=SCENE, dur=6.0,
    prompt=(
        # ★ 2026-09-16 修复（README §6.10）：storyboard 定**近景**；旧版写「特写」，
        #   成片是满画幅大脸（尾幕情绪镜，近景更留白、更适合「轻声说主题句」）。
        "CUT 1: 近景镜头，一位戴细框眼镜的初中女生的肩部以上入画，"
        "她先停顿了一下、轻轻吸了一口气，然后但很清楚地说出一句话；"
        "身后三位同学作为虚化的背景；"
        "四人的面貌、发型、眼镜与服装严格照 <Picture 1>（细框眼镜必须保留）；"
        "背景是那间傍晚已暗的教室，冷白的屏幕光打在她脸上；"
        + LIGHT + "。镜头缓慢向前推。\n"
        "Audio: 教室安静底噪（后期）；"
        "女生停顿一下、说：德才兼备。德是知道学一身本事为了谁。"
    ),
)
