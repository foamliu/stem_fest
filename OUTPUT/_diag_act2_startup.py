...
# ★★ 2026-09-18 新增：**全画面文字禁令**（README §6.10.11）
#   为什么需要"全画面"而不是只禁"全息光幕上"：
#     镜 18 验收读图发现设备机身烧着「豆包AI生成」六个字 —— 位置在画面下部
#     的道具表面，而旧禁令只写了「全息光幕上只有蓝紫色的光晕…没有任何文字」，
#     模型于是把文字挪到**没被禁的地方**（设备外壳）去渲。
#   ⇒ 禁令必须**穷举场景里的文字载体**，而不是只点名一个：
#     屏幕 / 道具表面 / 外壳 / 贴纸 / 标签 / 商标 / 丝印 / 印刷 / 水印 / 落款，
#     并且**明确点名"生成式 AI 水印"**（训练语料里最容易被翻出来的就是它）。
#   ⚠️ 与 §6.10.7 同一纪律：**只说"画面里不要有什么"，不说"你（模型）不要做什么"**。
NO_TEXT_SCENE = (
    "★ 全画面不得出现任何可读的文字、字幕、符号、数字或水印："
    "画面内的任何表面（设备外壳、机身、屏幕、桌面、墙面、纸面、衣物、道具、"
    "贴纸、标签、商标、丝印、印刷、落款、条码）上都没有任何文字、"
    "没有 logo、没有签名、没有水印、没有「AI 生成」一类的角标；"
    "全息光幕上只有蓝紫色的光晕与几何光带；画面干净得像未经印刷的实物照片。\n"
)
...
TASKS[17] = dict(
    ...
    prompt=(
        ...
        #   2026-09-16 那次「修复」把性别判反了（从 prompt 文字倒推外形），
        #   写成「初中男生（不戴眼镜）」⇒ 与女生定妆照冲突 ⇒ 成片配出男声。
        #   ⇒ 文字改回「戴细框眼镜的初中女生」，与 <Picture 1> 一致。
        "CUT 1: 特写镜头，画面主体是一位戴细框眼镜的初中女生的手指"
        "与课桌上那台银灰色全息设备的边缘：她的手指刚碰到设备边缘就被轻轻夹了一下，"
        "指节一缩、随即往回收；"
        "她的面貌、发型与服装严格照 <Picture 1>（白色短袖 Polo 衫 + 红领巾，"
        "十二三岁初中生的手，绝不是成年人的手）；"
        "背景是 <Picture 2> 那间傍晚的教室，暖金色天光与蓝紫冷光交叠、轻微虚化；"
        + LIGHT + "。镜头轻微向前缓推（slow dolly in）。\n"
        "Audio: 教室底噪，以及设备夹到手指的一声轻微「咔」（后期）；"
        "女生说：我查过了，模型只能看，不能改历史。"
    ),
)

TASKS[18] = dict(
    slug="sicheng_presses_start_button", seed=9633,
    ...
    prompt=(
        ...
        #
        # ★★ 2026-09-18 严重修复（README §6.10.11 **模型水印**）：
        #   验收读图发现成片里**设备机身印着「豆包AI生成」六个字**（清晰、可读、
        #   8 帧全中）—— 这是 H3 把训练语料里的**生成式 AI 水印**当场景元素
        #   画进了道具表面，属"烧进画面"的实体字，**字幕带扫描器抓不到**
        #   （位置在 0.72H，且在字幕带之外）。
        #   根因：本镜旧 prompt 是**全片唯一没有文字禁令的镜**（姊妹镜 15/16 都有），
        #        而且它偏偏是"特写一台机器"—— 特写道具 + 无禁令 = 模型自由发挥表面纹理，
        #        水印就是最容易被翻出来的"表面文字"。
        #   ⇒ ① 补上**全画面文字禁令**（且不再只限定在全息屏，改为"画面内任何表面"）；
        #     ② 明写设备外观约束（磨砂金属、无任何印刷/丝印/贴标/logo）；
        #     ③ 换 seed 9631 → 9633（水印是采样期决定的，换 seed 才能换掉）。
        # ★ 2026-09-19 更正（README 铁律 B6）：**刘思成是女生**（原写「男生（不戴眼镜）」= 判反了性别）。
        "CUT 1: 特写镜头。一位戴细框眼镜的初中女生（十二三岁的初中生年龄、"
